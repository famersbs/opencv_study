...
int facedetect_test_create_avi( const char* src_path, const char* result_file ){
	...
	int i = 0;
	const int max_frame = 169;
	char load_image_path[1024] = {0, };

	while(1) {
		++ i;

		if( i > max_frame ) break;
		sprintf( load_image_path, "%s/%08d.jpg", src_path, i );

		image = cvLoadImage( load_image_path );
		...
		if(VideoOut == NULL){
			VideoOut = cvCreateVideoWriter( result_file, CV_FOURCC('D', 'I', 'V', '3'), 1,
											cvGetSize(image),
											1);
		}

		// 이미지의 사이즈를 조정 하여 write 해야 하는 부분이다~!
		// 버퍼에 이미지 사이즈를 조정하여 write 한 뒤에 frame 을 써야 한다~!!
